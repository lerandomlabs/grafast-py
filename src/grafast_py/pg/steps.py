...
class PgSelectAllStep(PgCustomizable):
    ...
    def run_params(self) -> Optional[Dict[str, Any]]:
        ...
        returned. A variable-derived ``Placeholder`` rides a value-LESS bound param
        (``:root_first`` / ``:root_offset``); its runtime value is unwrapped here so the value
        lives only in the per-request params dict — the cached statement stays immutable.
        """
        params: Dict[str, Any] = {}
        if isinstance(self.first, Placeholder):
